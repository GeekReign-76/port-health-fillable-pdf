...
--->
<cfif not structKeyExists (variables,"encodeforerror")>
	<cfinclude template="../exception/udf.cfm">
</cfif>
<cfif IsDebugMode()>
<cfsilent>
...
<cfset undefined = "">
	...
	<!--- EVENT: ORM SQL Queries --->
	<cftry>
		<cfquery dbType="query" name="cfdebug_orm_queries" debug="false">
			SELECT *
			FROM qEvents
		...
		<cfcatch type="Any">
			<cfscript>
				cfdebug_orm_queries = queryNew('ATTRIBUTES, BODY, CACHEDQUERY, CATEGORY, DATASOURCE, ENDTIME, EXECUTIONTIME, LINE, MESSAGE, NAME, PARENT, PRIORITY, RESULT, ROWCOUNT, STACKTRACE, STARTTIME, TEMPLATE, TIMESTAMP, TYPE, URL, et');
			</cfscript>		
		</cfcatch>
	</cftry>
	...
	<cfparam name="bFoundORMSQLQueries" default="false" type="boolean">
	...
	<cftry>
		<cfscript>
			...
			if (IsDefined("cfdebug_orm_queries") AND cfdebug_orm_queries.RecordCount GT 0) { bFoundORMSQLQueries = true; displayDebug=true; }
			else { bFoundORMSQLQueries = false; }
...
<cfoutput>
	<cftry>
		...
		<tr>
			...
			<td class="cfdebug">#EncodeForErrorSmart(CGI.HTTP_USER_AGENT)#</td>
		</tr>
		<tr>
			<td class="cfdebug" nowrap> Remote IP </td>
			<td class="cfdebug">#EncodeForErrorSmart(CGI.REMOTE_ADDR)#</td>
		</tr>
		<tr>
			<td class="cfdebug" nowrap> Host Name </td>
			<td class="cfdebug">#EncodeForErrorSmart(CGI.REMOTE_HOST)#</td>
		</tr>
		</table>
		</p>
		<cfcatch type="Any"></cfcatch>
    ...
    <cfif cfdebug_top_level_execution_sum.recordCount and len(trim(cfdebug_top_level_execution_sum.executionTime[1])) gt 0>
        ...
        <cfif cfdebugger.settings.template_mode EQ "tree">
            ...
            <cfloop index="i" from="1" to="#arrayLen(a)#">
                <cfset childidList = "">
                <cfset parentidList = "">
                <cfloop index="x" from="#arrayLen(a[i].stacktrace)#" to="1" step="-1">
                    <cfscript>
                        if( a[i].stacktrace[x].id NEQ "CF_INDEX" ) {
                            // keep appending the line number from the template stack to form a unique id
                            childIdList = listAppend(childIdList, a[i].stacktrace[x].line);
                            if( x eq 1 ) {
                                //parentIdList = listAppend(parentIdList, a[i].stacktrace[x].template);
								raw_trace = a[i].stacktrace[x].raw_trace;
                                ...
                                if( findFunction.len[1] NEQ 0 AND findFunction.pos[1] NEQ 0 ) {
                                    ...
                                    parentIdList = listAppend(parentIdList, a[i].stacktrace[x].template & " | " & lcase(parentfunction));
                                }
								else {
                                	parentIdList = listAppend(parentIdList, a[i].stacktrace[x].template);
								}
                                ...
                                parentIdList = listAppend(parentIdList, a[i].stacktrace[x].line);
                            }
                        }
                    </cfscript>
                </cfloop>
        	...
        	<cftry>
                ...
                <cftry>
                    <cfoutput query="cfdebug_templates_summary">
                        <cfset templateOutput = encodeForErrorSmart(template)>
                        <cfset templateAverageTime = Round(encodeForErrorSmart(totalExecutionTime) / instances)>
                        ...
                        <cfif template EQ ExpandPath(cgi.script_name)>
                            <cfset templateOutput = "<img src='#getpageContext().getRequest().getContextPath()#/CFIDE/debug/images/topdoc.gif' alt='top level' border='0'> " &
							 ...
							 <cfif templateAverageTime GT cfdebugger.settings.template_highlight_minimum>
                                <cfset templateOutput = "<font color='red'><span class='template_overage'>" & encodeForErrorSmart(template) & "</span></font>">
                                <cfset templateAverageTime = "<font color='red'><span class='template_overage'>" & templateAverageTime & "</span></font>">
								<cfset totalTime = "<font color='red'><span class='template_overage'>" & encodeForErrorSmart(totalExecutionTime) & "</span></font>">
                            </cfif>
                        <cfelse>
                            <cfif templateAverageTime GT cfdebugger.settings.template_highlight_minimum>
                                <cfset templateOutput = "<font color='red'><span class='template_overage'>" & encodeForErrorSmart(template) & "</span></font>">
                        ...
                        <tr>
							<cfif isDefined("totalTime") and len(trim(totalTime))>
								...
								<cfset totalTime = "">
							<cfelse>
    	                        <td align="right" class="cfdebug" nowrap>#encodeForErrorSmart(totalExecutionTime)# ms</td>
							</cfif>
                            <td align="right" class="cfdebug" nowrap>#templateAverageTime# ms</td>
                            <td align="center" class="cfdebug" nowrap>#encodeForErrorSmart(instances)#</td>
                ...
                <cfoutput>
                <tr>
					<td align="right" class="cfdebug" nowrap><i>#encodeForErrorSmart(time_other)# ms</i></td><td colspan=2>&nbsp;</td>
                    <td align="left" class="cfdebug"><i>STARTUP, PARSING, COMPILING, LOADING, &amp; SHUTDOWN</i></td>
				</tr>
                <tr>
					<td align="right" class="cfdebug" nowrap><i>#encodeForErrorSmart(cfdebug_execution.executionTime)# ms</i></td><td colspan=2>&nbsp;</td>
...
<!--- ORM SQL Queries --->
<cfoutput>
<cfif bFoundORMSQLQueries>
	<cftry>
		<p class="cfdebug"><hr/><b class="cfdebuglge"><a name="cfdebug_orm_queries">ORM SQL Queries</a></b></p>
		<cfloop query="cfdebug_orm_queries">
			<pre>#htmleditformat(cfdebug_orm_queries.body)#</pre>			
		</cfloop>
	<cfcatch type="Any">
...
<code><b>#cfdebug_storedproc.name#</b> (Datasource=#cfdebug_storedproc.datasource#, Time=#Max(cfdebug_storedproc.executionTime, 0)#ms) in #cfdebug_storedproc.template# @ #TimeFormat(cfdebug_storedproc.timestamp, "HH:mm:ss.SSS")#</code><br />
    ...
    <tr>
        ...
        <td>
            ...
            <tr>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "type")>#encodeForErrorSmart(thisParam.type)#</cfif></code></td>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "sqlType")>#encodeForErrorSmart(thisParam.sqlType)#</cfif></code></td>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "value")>#htmleditformat(thisParam.value)#</cfif></code></td>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "variable")>#encodeForErrorSmart(thisParam.variable)# = #CFDebugSerializable(thisParam.variable)#</cfif></code></td>
                <td>&nbsp;<code><cfif StructKeyExists(thisParam, "dbVarName")>#encodeForErrorSmart(thisParam.dbVarName)#</cfif></code></td>
            </tr>
            </cfloop>
            </table>
        </td>
...
<cfloop query="cfdebug_timer">
    <cftry>
    	<img src='#getpageContext().getRequest().getContextPath()#/CFIDE/debug/images/#Replace(cfdebug_timer.priority, " ", "%20")#_16x16.gif' alt="#cfdebug_timer.priority# type">
		 [#val(cfdebug_timer.endTime) - val(cfdebug_timer.startTime)#ms] <i>#encodeForErrorSmart(cfdebug_timer.message)#</i><br />
    	<cfcatch type="Any"></cfcatch>
    </cftry>
</cfloop>
</p>
