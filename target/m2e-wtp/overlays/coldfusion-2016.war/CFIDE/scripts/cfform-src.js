...
_CF_checkinteger = function(object_value, required,isstrict)
{
	//trim whitespace before we validate
    object_value = object_value.replace(/^\s+/,'').replace(/\s+$/,'');
	...
	if (check_char == -1)
		return _CF_checknumber(object_value,required,isstrict);
	else
		return false;
}
...
_CF_checknumber = function(object_value, required,isstrict)
{
